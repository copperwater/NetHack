   ...
   if debug_rm_idx ~= nil then
      ...
      if percent(50) then
         if is_eligible(themerooms[debug_rm_idx]) then
            ...
            pline("Warning: specified themeroom is not eligible")
         end
      end
...
function pre_themerooms_generate()
   ...
   if debug_themerm ~= nil and debug_rm_idx == nil then
      pline("Warning: specified themeroom not found in themerooms", true)
   end
   if debug_fill ~= nil and debug_fill_idx == nil then
      pline("Warning: specified themeroom fill not found in themeroom_fills", true)
   end
end
...
function themeroom_fill(rm)
   if debug_fill_idx ~= nil then
      if is_eligible(themeroom_fills[debug_fill_idx], rm) then
         ...
         pline("Warning: specified fill is not eligible in room that generated it.")
      end
      return
   end
   local pick = nil;
